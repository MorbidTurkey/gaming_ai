...
class GamingChatbotAgent:
    ...
    def __init__(self):
        ...
        if self.gamalytic_api.is_available:
            ...
            self.available_functions["get_player_affinity"] = self.get_player_affinity
        ...
        if self.gamalytic_api.is_available:
            gamalytic_tools = [
                ...
                },
                {
                    "type": "function",
                    "function": {
                        "name": "get_player_affinity",
                        "description": "Get games that players of a specific game also play most often. Perfect for 'What other games do [Game] players play?' queries",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "game_name": {
                                    "type": "string",
                                    "description": "Name of the game to analyze player affinity for"
                                },
                                "limit": {
                                    "type": "integer",
                                    "description": "Number of similar games to return (default 10)",
                                    "default": 10
                                }
                            },
                            "required": ["game_name"]
...
- get_player_affinity(game_name) → Games that players of a specific game also play
...
- "What other games do [Game] players play?" → get_player_affinity(game_name)
...
- "Games that [Game] players also enjoy" → get_player_affinity(game_name)
...
Or fix the OpenAI connection for full AI-powered responses!"""
    ...
    def get_player_affinity(self, game_name: str, limit: int = 10) -> Dict:
        """Get games that players of the specified game also play"""
        try:
            # Try Gamalytic API first for real player affinity data
            if self.gamalytic_api.is_available:
                result = self.gamalytic_api.get_player_affinity(game_name, limit)
                if result.get("success"):
                    return result
            
            # Fallback: Use alternative approach combining multiple APIs
            return self._get_player_affinity_fallback(game_name, limit)
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def _should_auto_visualize(self, function_calls: List[str], user_message: str) -> bool:
        ...
        data_functions = [
            ...
            "get_player_affinity", "get_game_audience_overlap", "get_similar_games_by_players"  # Added player affinity functions
        ]
    ...
    def _determine_visualization_params(self, function_calls: List[str]) -> Tuple[str, str]:
        ...
        elif "get_player_affinity" in function_calls or "get_similar_games_by_players" in function_calls:
